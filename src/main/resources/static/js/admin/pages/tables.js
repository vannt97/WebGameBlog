$(document).ready(()=>{
        ...
        getApi("blogs",(res)=>{
            ...
            $(".btn-remove").click(function (e){
                if (confirm("Bạn có muốn xoá không?") === true) {
                    deleteApi(`delete/${e.currentTarget.dataset.id}/blog`,(res)=>{
    ...
    if(window.location.pathname === "/admin/accounts"){
        getApi("accounts",(res)=>{
            if(res.status !== 200) return;
            let htmlTitle = "<th>Id</th><th>Name</th><th>Role</th><th>Created At</th><th>Updated At</th><th>Action</th>";
            let htmlFooterTable = `<thead><tr>${htmlTitle}</tr></thead>`;
            let htmlHeaderTable = `<tfoot><tr>${htmlTitle}</tr></tfoot>`;
    ...
    if(window.location.pathname === "/admin/categories"){
        getApi("categories",(res)=>{
            if(res.status !== 200) return;
            let htmlTitle = "<th>Id</th><th>Name</th><th>Status</th><th>Created At</th><th>Updated At</th><th>Action</th>";
            let htmlFooterTable = `<thead><tr>${htmlTitle}</tr></thead>`;
            let htmlHeaderTable = `<tfoot><tr>${htmlTitle}</tr></tfoot>`;
            ...
            $(".btn-remove").click(function (e){
                if (confirm("Bạn có muốn xoá không?") === true) {
                    deleteApi(`delete/${e.currentTarget.dataset.id}/category`,(res)=>{
                        alert("Xoá thành công");
            ...
            document.getElementById("edit-category-form").addEventListener('submit',function (e){
                e.preventDefault();
                const data = Object.fromEntries(new FormData(e.target).entries());
                postApi('edit/category',JSON.stringify(data),(res)=>{
            ...
            document.getElementById('create-category-form').addEventListener('submit',function (e){
                e.preventDefault();
                const data = Object.fromEntries(new FormData(e.target).entries());
                postApi('create/category',JSON.stringify(data),(res)=>{
